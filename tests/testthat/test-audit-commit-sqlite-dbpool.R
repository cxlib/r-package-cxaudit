...
testthat::test_that( "audit.commit.auditEnabledSQLiteDbPoolSingleRecord", {
  ...
  test_dbpool <- base::get( "testdbpool", envir = cxaudit_test_env )
  
  if ( base::nrow( DBI::dbGetQuery( test_dbpool, paste( "select cast( uid as varchar(128) ) as uid from tbl_adt_records ;" ) ) ) > 0 )
  ...
  # - test store
  test_store <- cxaudit::cxaudit_store( test_dbpool )
  
  
  # -- test
  #' @cx.tests Commit single record to SQLite database pool with fail cache enabled
...
testthat::test_that( "audit.commit.auditEnabledSQLiteDbPoolMultiRecord", {
  ...
  # -- test
  #' @cx.tests Commit multiple records to SQLite database pool with fail cache enabled
...
testthat::test_that( "audit.commit.auditEnabledSQLiteDbPoolMultiRecordFailCacheDisabled", {
  ...
  # -- test
  #' @cx.tests Commit multiple records to SQLite database pool with fail cache disabled
  result <- cxaudit::cxaudit_commit( test_rec_objs, store = test_store )
