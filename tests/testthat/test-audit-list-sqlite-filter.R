...
testthat::test_that( "audit.list.auditEnabledSQLiteNoRecordsSimpleArgCriteria", {
  ...
  if ( ! cxaudit::cxaudit_validevent( test_event ) ) 
    testthat::fail( "Unexpected invalid event selected" )
  ...
  result <- cxaudit::cxaudit_list( events = test_event,  store = test_store )
...
testthat::test_that( "audit.list.auditEnabledSQLiteMultiRecordsCriteriaLimitArg", {
  ...
  # -- test
  #' @cx.tests List audit records with limit filter criteria as arguments from SQLite database with number of records greater than default limit
  result <- cxaudit::cxaudit_list( limit = test_limit, store = test_store )
  
  
  # -- expected
  
  # note: test_rec_objs are chronologically descending
  expected_objs <- utils::head( test_rec_objs, n = test_limit )
  ...
  # - result
  testthat::expect_true( base::is.list(result) )
  testthat::expect_length( result, test_limit )
...
testthat::test_that( "audit.list.auditEnabledSQLiteMultiRecordsCriteriaLimitArgList", {
  ...
  # -- test
  #' @cx.tests List audit records with limit filter criteria as list from SQLite database with number of records greater than default limit
  result <- cxaudit::cxaudit_list( list( "limit" = test_limit), store = test_store )
...
testthat::test_that( "audit.list.auditEnabledSQLiteMultiRecordsCriteriaLimitSelectFirstArg", {
  ...
  # - limit
  test_limit <- 40
  testthat::expect_true( test_limit < length(test_rec_objs) )
  
  
  
  # -- test
  #' @cx.tests List audit records in chronological order with limit filter criteria as arguments from SQLite database with number of records greater than limit 
  result <- cxaudit::cxaudit_list( limit = test_limit, select = "first", store = test_store )
  
  
  # -- expected
  
  # note: test_rec_objs are chronologically descending ... so first (asdcending order) is the tail part 
  expected_objs <- utils::tail( test_rec_objs, n = test_limit )
  base::names(expected_objs) <- lapply(expected_objs, function(x) { x$getproperties()[["id"]] } )
  ...
  # note: expecting max limit
  testthat::expect_equal( as.numeric(result_filter[["limit"]]), length(expected_objs) )
  testthat::expect_equal( result_filter[["select"]], "first")
